...
function getStringLength(value) {
  ...
  const numberOfString = value.length;
...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.slice(0, indexOfSubstring) + str.slice(indexOfSubstring + value.length)
  );
}
...
function formatTime(minutes, seconds) {
  ...
  return currentMinutes.concat(':', currentSeconds);
}
...
function countVowels(str) {
  ...
  const newCurrentString = str.toLowerCase().split('');
  newCurrentString.forEach((elem) => {
...
function invertCase(str) {
  ...
  let i = 0;
  while (i < str.length) {
    let invertChar = str.charAt(i);
    if (invertChar === invertChar.toUpperCase()) {
      invertChar = invertChar.toLowerCase();
    } else {
      invertChar = invertChar.toUpperCase();
    }
    i += 1;
    newInvertString += invertChar;
  }
  return newInvertString;
}
...
function extractNameFromTemplate(value) {
  const splitString = value.replace(/[^a-zа-яё0-9\s]/gi, '').split(' ');
